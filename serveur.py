            ...
            if sliced_msg[2]=="G94" and int(sliced_msg[1][1:2]) == infoGame[2]: #donnée envoyé au module erronée
                ...
                if int(sliced_msg[1][1]) == infoGame[2]:
                    if sliced_msg[2]=="G24": #flotte coulée
                        publish(client,sliced_msg[0]+"/"+infoGame[pubPlayer-1]+"/In","G24")
                ...
                elif sliced_msg[2] =="G06" and infoGame[4] in ["G99","G98","G45","G44"]:
                    if infoGame[5]=="G06" :
                        print("newparty")
                        newGame(sliced_msg,infoGame)
                    else:
                        publish(client,sliced_msg[0]+"/"+infoGame[receiver-1]+"/In","G06")
...
def checkMAC(MAC):
    ...
    if not unassigned:
        ...
        publish(client,party+"/"+idAssigned+"/In","G05")
    return unassigned
    ...
    for(IdGame,IdJ1,IdJ2,Timestamp,IntervalTime,Status) in cur:                   
        if Status != "FINISHING" and Timestamp and IntervalTime:  
            if dt-int(Timestamp) >= int(IntervalTime):
                ...
                Id=IdGame
                listToUp.append(Id)
                writeLog("logId.txt",IdGame+" finished")
